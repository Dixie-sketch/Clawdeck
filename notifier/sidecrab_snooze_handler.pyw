...
def main(argv: list[str] | None = None) -> int:
    ...
    raw = argv[0]
    session_id = parse_snooze_uri(raw)
    if session_id is None:
        ...
        #
        # The isinstance guard is not decoration. parse_snooze_uri takes Any and is documented
        ...
        log_line(f"refused: argument ({shape}) is not {SNOOZE_SCHEME}:{SESSION_ID_PATTERN}")
        return EXIT_BAD_URI
